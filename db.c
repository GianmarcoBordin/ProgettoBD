...
bool setup_prepared_stmt(MYSQL_STMT **stmt, char *statement, MYSQL *conn)
{
	...
	/* primo passo allocare un gestore di prepared statement*/

	*stmt = mysql_stmt_init(conn);
	if (*stmt == NULL)
	{
		print_error(conn, "Non è stato possibile inizializzare lo statement poichè mysql_stmt_init() è fallita\n");
		return false;
	}
	
	/* secondo passo inviare statement al serever per precompilarlo*/

	if (mysql_stmt_prepare (*stmt, statement, strlen(statement)) != 0) {
		print_stmt_error(*stmt, "Non è stato possibile inizializzare lo statement poichè mysql_stmt_prepare() è fallita\n");
		return false;
	}
	
	/* poi diciamo a mysql_stmt_store_result di aggiornare il valore del campo MYSQL_FIELD->max_length*/
	mysql_stmt_attr_set(*stmt, STMT_ATTR_UPDATE_MAX_LENGTH, &update_length);

	return true;
}

//NON USATA DA TOGLIERE
